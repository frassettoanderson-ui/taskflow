...
interface ContatoAv { nome: string; avaliacoes: number; media: number; ultima: string }
interface EmpresaAv { id: string; nome: string; detratores: number; neutros: number; promotores: number; avaliacoes: number; media: number; contatos: ContatoAv[] }
interface Cards { detratoras: number; neutras: number; promotoras: number; nps: number }
interface Resp { cards: Cards; empresas: EmpresaAv[] }
...
function Estrelas({ media }: { media: number }) {
  ...
  return (
    <span className="whitespace-nowrap text-[14px] leading-none">
      {Array.from({ length: 5 }, (_, i) => <span key={i} className={i < cheias ? 'text-emerald-500' : 'text-slate-300'}>★</span>)}
    </span>
  );
}

function CardAv({ cor, pct, label, onClick }: { cor: string; pct: number; label: string; onClick?: () => void }) {
...
export default function AvaliacoesSolicitacoes() {
  ...
  useEffect(() => {
    api.get<Resp>('/gestao-portal/avaliacoes-solicitacoes')
      .then(setD)
    ...
    <div className="-m-6 min-h-full bg-fundo p-4 text-[13px]">
      ...
      <div className="mb-2 flex items-center justify-between">
        <div className="flex items-center gap-2 text-slate-600">
          ...
          <span className="font-medium text-slate-700">Avaliacao das Solicitacoes</span>
        </div>
      ...
      <div className="mb-3 flex flex-wrap gap-2">
        <CardAv cor={COR.detr} pct={d.cards.detratoras} label="Empresas Detratoras" onClick={() => setFlags({ detr: true, neut: false, prom: false })} />
        <CardAv cor={COR.neut} pct={d.cards.neutras} label="Empresas Neutras" onClick={() => setFlags({ detr: false, neut: true, prom: false })} />
        <CardAv cor={COR.prom} pct={d.cards.promotoras} label="Empresas Promotoras" onClick={() => setFlags({ detr: false, neut: false, prom: true })} />
        <CardAv cor={COR.nps} pct={d.cards.nps} label="Avaliacao" onClick={() => setFlags({ detr: true, neut: true, prom: true })} />
      </div>
