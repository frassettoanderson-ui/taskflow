...
const COR = { ok: '#88b87f', info: '#69a8d9', warn: '#ffb752', danger: '#d15b47', roxo: '#9585bf' };
...
function Ctrl({ children, onClick, title }: { children: React.ReactNode; onClick: () => void; title: string }) {
  return <button onClick={onClick} title={title} className="grid h-8 w-9 place-items-center rounded text-slate-500 hover:bg-fundo">{children}</button>;
}
  ...
  return (
    <div className="card overflow-hidden">
      <div className="flex items-center gap-3 border-b border-slate-100 p-3">
        <div className="grid h-12 w-12 place-items-center overflow-hidden rounded-full bg-fundo text-slate-400" style={it.cor ? { background: it.cor + '22', color: it.cor } : undefined}>
...
function Barra({ label, valor, pct, cor }: { label: string; valor: number; pct: number; cor: string }) {
  return (
    <div>
      ...
      <div className="h-2 w-full overflow-hidden rounded bg-fundo"><div className="h-full" style={{ width: `${pct}%`, background: cor }} /></div>
    </div>
  );
}
