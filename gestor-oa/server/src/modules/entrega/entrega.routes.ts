...
const listQuery = z.object({
  ...
  entAntecipada: z.string().optional(),
  entNoPrazoTec: z.string().optional(),
  entAtrasada: z.string().optional(),
  pendAntesTec: z.string().optional(),
  pendDentroTec: z.string().optional(),
  soEntreguesPeloResp: z.string().optional(),
...
router.get('/', validate({ query: listQuery }), async (req, res) => {
  ...
  const { items, total } = await svc.listar(
    req.auth!.escritorioId,
    {
      ...
      entAntecipada: q.entAntecipada === 'true',
      entNoPrazoTec: q.entNoPrazoTec === 'true',
      entAtrasada: q.entAtrasada === 'true',
      pendAntesTec: q.pendAntesTec === 'true',
      pendDentroTec: q.pendDentroTec === 'true',
      soEntreguesPeloResp: q.soEntreguesPeloResp === 'true',
