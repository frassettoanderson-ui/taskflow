...
Cada `.hero`/`.secao` ganha um SVG `.vinha` num dos lados (alternando), **gerado pelo JS** em `betel.js` — caule serpenteante (curvas cúbicas com semente fixa por seção, então não muda a cada carregamento), folhas (`#lg-folha`), margaridas (`#lg-flor`) e botões (`#lg-botao`) distribuídos por `getPointAtLength`. Os três glifos são **desenhados no estilo do ramo da marca** — silhueta com borda grossa e vazado interno, `fill-rule:evenodd` — e gerados por script (a folha é uma lente levemente curvada em torno de uma linha central; a primeira versão, um losango cheio, ficou "quadrada" e o cliente rejeitou). O progresso da seção na janela (0 = topo encostando na base da janela, 1 = base da seção a 40% da janela) é suavizado e escrito em `--p`; o caule se desenha via `stroke-dashoffset` e cada broto, que tem `--t` (posição no caule), cresce quando `--p` passa de `--t` (`transform:scale(var(--s))` com `transform-box:fill-box`, origem no pé da folha). **Rolar para cima desfaz** — é scrub, não gatilho. Replanta em `resize` e em `load` (as alturas mudam quando fontes/fotos chegam).
